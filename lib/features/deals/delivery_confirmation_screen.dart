...
class _DeliveryConfirmationScreenState extends ConsumerState<DeliveryConfirmationScreen> {
    ...
    return FxPageScaffold(
      ...
      body: dealAsync.when(
        ...
        data: (deal) {
          ...
          final estProfit = deal.costBasisPkr != null ? deal.customerPayablePkr - deal.costBasisPkr! : null;
              ...
              children: [
                FxObsidianReportPanel(
                  child: Column(
                    ...
                    children: [
                      Text('Deal ${deal.dealNo}', style: AppTypography.headlineMd(context.fx.onSurface, context: context)),
                      Text('Customer: ${deal.customerName ?? '—'}', style: AppTypography.bodyMd(context.fx.onSurfaceVariant, context: context)),
                      Text('Sale: PKR ${fmt.format(deal.customerPayablePkr)}', style: AppTypography.bodyMd(context.fx.onSurface, context: context)),
                      if (estProfit != null) Text('Est. profit: PKR ${fmt.format(estProfit)}', style: AppTypography.bodyMd(context.fx.primary, context: context)),
                    ],
                  ),
                ),
                ...
                FxObsidianFormField(controller: _proofCtrl, label: 'Proof / TT reference'),
  ...
  Future<void> _submit() async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(dealRepositoryProvider).confirmDelivery(

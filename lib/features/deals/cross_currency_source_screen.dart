...
class CrossCurrencySourceScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CrossCurrencySourceScreen> createState() =>
      _CrossCurrencySourceScreenState();
}

class _CrossCurrencySourceScreenState
    extends ConsumerState<CrossCurrencySourceScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final agentLegs =
        timelineAsync.whenOrNull(
          data: (legs) => legs
              .where((l) => l.legType == FxDealLegType.agentSource)
              .toList(),
        ) ??
        [];
        ...
        child: ListView(
          ...
          children: [
            ...
            FxObsidianFormField(
              ...
              validator: (v) =>
                  double.tryParse(v ?? '') == null ? 'Enter PKR amount' : null,
            ),
            FxObsidianFormField(
              ...
              validator: (v) =>
                  double.tryParse(v ?? '') == null ? 'Enter AED amount' : null,
            ...
            if (agentLegs.isNotEmpty) ...[
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(
                  labelText: 'AED delivered to (Agent C leg)',
                ...
                items: agentLegs
                    .map(
                      (l) => DropdownMenuItem(
                        value: l.id,
                        child: Text(
                          'Leg ${l.legNo}: ${l.counterpartyName ?? 'Agent'}',
                        ),
                      ),
                    )
                    .toList(),
  ...
  Future<void> _submit(List<FxDealLeg> agentLegs) async {
    ...
    if (targetLegId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Add an agent source leg first')),
      );
      return;
    }
    setState(() => _busy = true);
    try {
      final fromLegId = await ref

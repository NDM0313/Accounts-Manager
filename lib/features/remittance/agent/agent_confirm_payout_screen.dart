...
class AgentConfirmPayoutScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AgentConfirmPayoutScreen> createState() => _AgentConfirmPayoutScreenState();
}

class _AgentConfirmPayoutScreenState extends ConsumerState<AgentConfirmPayoutScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(remittanceRepositoryProvider).agentConfirmPayout(
        ...
        child: ListView(
          children: [
            ...
            ListTile(
              ...
              onTap: () async {
                final date = await showDatePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime.now().add(const Duration(days: 1)), initialDate: _payoutAt);
                if (date == null || !mounted) return;
                final time = await showTimePicker(context: context, initialTime: TimeOfDay.fromDateTime(_payoutAt));
                if (time == null) return;
                setState(() => _payoutAt = DateTime(date.year, date.month, date.day, time.hour, time.minute));
              },
            ),

...
class ProfileNotConfiguredScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final userEmail = ref.watch(authSessionProvider).maybeWhen(
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Center(
            child: Icon(Icons.admin_panel_settings_outlined, size: 56, color: theme.colorScheme.primary),
          ),
          ...
          if (userEmail != null) ...[
            const SizedBox(height: 8),
            Text(userEmail, textAlign: TextAlign.center, style: theme.textTheme.bodyMedium),
          ],
          ...
          _SetupStep(
            ...
            body: 'Copy the UUID above, or open Supabase → Authentication → Users.',
          ),
          _SetupStep(
            number: 2,
            title: 'Run bootstrap SQL',
            body: 'Open SQL Editor on project ygidlcqhupmxvsdjmvnf. '
                'Run $_sqlScriptPath (replace YOUR_AUTH_USER_UUID_HERE).',
          ),
          _SetupStep(
            number: 3,
            title: 'Assign FXDEV / MAIN + admin role',
            body: 'The script links you to company FXDEV, branch MAIN, and the admin role. '
                'See $_bootstrapDocPath for manual Table Editor steps.',
          ),
          _SetupStep(
            number: 4,
            title: 'Return to app',
            body: 'Tap refresh (top right) or sign out and sign in. Dashboard should open with COA and currencies.',
          ),
          const SizedBox(height: 24),
          Card(
            child: Padding(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('YOUR USER ID (auth.users.id)', style: AppTypography.labelCaps(context.fx.onSurfaceVariant, context: context)),
          const SizedBox(height: 8),
          SelectableText(userId, style: AppTypography.labelMono(context.fx.onSurface, context: context)),
...
class _SetupStep extends StatelessWidget {
  const _SetupStep({required this.number, required this.title, required this.body});

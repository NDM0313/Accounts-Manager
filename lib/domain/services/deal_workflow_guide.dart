  ...
  static DealWorkflowView build({
    ...
    steps.add(
      DealWorkflowStep(
        ...
        amountLabel: order != null
            ? '${deal.sellAmount} ${deal.sellCurrencyCode}'
            : null,
      ),
    );
      ...
      steps.add(
        DealWorkflowStep(
          ...
          status: sourcing?.status == FxDealLegStatus.completed
              ? DealWorkflowStepStatus.completed
              : DealWorkflowStepStatus.pending,
          amountLabel: sourcing != null
              ? '${sourcing.receiveAmount} ${sourcing.receiveCurrency ?? deal.sellCurrencyCode}'
              : null,
    ...
    if (deal.status == FxDealStatus.completed) {
      ...
      nextRoute = null;
        ...
        (pending(FxDealLegType.sourcingRequirement) &&
            !has(FxDealLegType.agentSource))) {
      nextTitle = 'Source currency from agent';
      nextRoute = '/deals/${deal.id}/legs/agent-source';
      warning =
          '${deal.sellCurrencyCode} may not be available in own balance — add agent source leg.';
    ...
    } else if (has(FxDealLegType.agentPayment) &&
        !has(FxDealLegType.currencyReceipt)) {
      nextTitle = 'Confirm currency received';
      nextRoute = '/deals/${deal.id}/legs/currency-receipt';
    } else if (!has(FxDealLegType.delivery) &&
        deal.status != FxDealStatus.completed) {
